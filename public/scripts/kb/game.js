$(document).ready(function() {
	...
	Crafty.load(["/images//kb/sprite.png", "/images/kb/bg.png"], function() {
		//splice the spritemap
		Crafty.sprite(64, "/images/kb/sprite.png", {
			ship: [0,0],
			big: [1,0],
			medium: [2,0],
			small: [3,0]
	...
	Crafty.scene("main", function() {
		Crafty.background("url('/images/kb/bg.png')");

		//audio file
		var audio = new Audio('/images/kb/sound.wav');

		//check for bk logo
		var bktest = document.getElementById('bk');

		//delete bk div if exists
		if (bktest){
			$('#bk').remove();
		}
		...
		var player = Crafty.e("2D, Canvas, ship, Controls, Collision")
			...
			.bind("keydown", function(e) {
				...
				if(player.score == 200 && prize == false){
					bkEgg();
				}
		...
		function bkEgg() {
			...
			bkdiv.id = "bk";
			var objTo = document.getElementById('cr-stage');
			objTo.appendChild(bkdiv);
 			audio.play();
		    $('#bk').animate({
			  opacity: .01,
			  'height': '1200px'
